...
// MAIN CODE
void Grid3D_light_NoCoincident(){
    bool DoFit = true;
    ...
    */
    ...
    //Niters = 100*100;;  // For testing purposes, only look at first 10000 events
    ...
    for (int i=0; i<Niters; i++){
        ...
        if(abs(InteractionTime)<1.3e6){
            ...
            if( RecoFlash==true ){
                TVector3 vertex_pos;
                vertex_pos.SetXYZ(dEPromX, dEPromY, dEPromZ);
                // Save the eventID and interaction time of the coincident interaction
    ...
    std::sort(
        ID_interactions_flashed.begin(),
        ID_interactions_flashed.end(),
        [](const Event_Info_flashed& a, const Event_Info_flashed& b) {
            if (a.run_ID_struct    != b.run_ID_struct)
                return a.run_ID_struct < b.run_ID_struct;
            if (a.subrun_ID_struct != b.subrun_ID_struct)
                return a.subrun_ID_struct < b.subrun_ID_struct;
            if (a.event_ID_struct  != b.event_ID_struct)
                return a.event_ID_struct < b.event_ID_struct;
            return a.time_struct < b.time_struct;   // secondary sort
        }
    );
    /*
    cout << "\n===== ID_interactions_flashed (sorted) =====\n";
    for (const auto& inter : ID_interactions_flashed) {
        cout
            << "Run: "     << inter.run_ID_struct
            << "  SubRun: " << inter.subrun_ID_struct
            << "  Event: "  << inter.event_ID_struct
            << "  Time [us]: " << inter.time_struct
            << "  PDG: " << inter.pdgcode_struct
            << "  Vertex (x,y,z): ("
            << inter.vertex_struct.X() << ", "
            << inter.vertex_struct.Y() << ", "
            << inter.vertex_struct.Z() << ")"
            << endl;
    }
    */
    ...
    for(string file : filenames){
        ...
        int Niters = num_events;
        Niters = 20;  // For testing purposes, only look at first 100 events
        for (int i=0; i<Niters; i++){
            ...
            for(int f=0; f<nopflash; f++){
                ...
                */
                ...
                if(coincident_interaction==false){  // I only want to plot the events that have DO NOT HAVE A coincident interaction with the OpFlash
                    ...
                    for (const auto& inter : ID_interactions_flashed) {
                        if(inter.event_ID_struct == eventID
                            && inter.run_ID_struct == runID 
                            && inter.subrun_ID_struct == subrunID
                            && abs(inter.time_struct - flash_time->at(f)) < 6.) {   // If the eventID matches and the time of the interaction is close to the time of the OpFlash within the veto window
                            
                            //cout<<"Found an interaction that caused an OpFlash! in interaction number: "<<i<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl;
                    ...
                    TPaveText *pt = new TPaveText(0.45, 0.75, 0.9, 0.9, "NDC");  
                    ...
                    if(DoFit==true){
                        ...
                        // I save the canvas
                        c1->SaveAs(Form("Distributions_3D_NoCoincident_fit/Distribution_%i_%i.pdf", i, f));
                    }
                    ...
                    // I save the canvas
                    c1->SaveAs(Form("Distributions_3D_NoCoincident/Distribution_%i_%i.pdf", i, f));
